...
def create_cbo(
    ...
    def update_params(
        ...
        Vel = jax.tree_map(lambda v, x, y: v - learning_rate * (x- y) * m + 
                           - learning_rate* gamma2 *m* v +
                              sigma*jnp.sqrt(learning_rate*m)*jax.random.normal(key, x.shape)
                              , Vel, params, M_params_hat)
        rng, key = jax.random.split(key)
        params = jax.tree_map(lambda v, x, y: x + learning_rate* v 
                              - learning_rate* gamma1 * (x-y) +
                                sigma*jnp.sqrt(learning_rate)*jax.random.normal(key, x.shape)
                              , Vel, params ,M_params_hat)
