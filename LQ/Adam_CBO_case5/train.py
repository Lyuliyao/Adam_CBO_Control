...
parser.add_argument('--batch_size', type=int, default=5, help='batch_size of the problem.')
...
def create_params_update(update_fn,
    ...
    def params_update(i: int, input: Tuple[dict, dict, jax.random.PRNGKey, jnp.ndarray]) -> Tuple[dict, dict, jax.random.PRNGKey, jnp.ndarray]:
        ...
        updata_indices = rng_perm[:N_CBO_batch]
        params_perm = jax.tree_map(lambda x: jnp.take(x,updata_indices , axis=0), params)
        rng, key = jax.random.split(rng)
