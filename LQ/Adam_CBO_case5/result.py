...
parser.add_argument('--batch_size', type=int, default=5, help='Dimension of the problem.')
args = parser.parse_args()
batch_size = args.batch_size
config  = generate_configure(args.batch_size)
# Set up logging
...
params= np.load(f"./result_{batch_size}/params.npy", allow_pickle=True)
params_new = []
...
loss = np.load(f"./result_{batch_size}/value.npy", allow_pickle=True)
np.savez(f"./result_{batch_size}/result.npz", loss=loss, cbo_value=cbo_value, y_star=y_star)
