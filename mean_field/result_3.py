...
def generate_control_loss(
    ...
    def mixture_two_gaussian(key):
        ...
        # Generate Y ~ N(0, 1) normal random variable
        key, subkey = jax.random.split(key)
        Y = jax.random.normal(subkey)

        # Determine the floor of 3U
        U_floor = jnp.floor(3 * U)

        # Mixture of three Gaussians:
        X0 = (-1 * (U_floor == 0) * k + (U_floor == 1) * k) + theta * Y
        return X0
...
value_test2 = problem_configure["fcn_Qs"](x)*N_mv+N_mv*problem_configure["fcn_Q"](x)*0.0648
np.savez(f"./result_{dim}/value_test_mv_{N_mv}_gaussian_3.npz",value_test1=value_test1,value_test2=value_test2)
