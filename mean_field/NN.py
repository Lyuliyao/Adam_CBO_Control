...
def create_nn(
    ...
    Args:
        nn_size: Size of the neural network ensemble.
        control: Dictionary containing the control layer settings.
        distribution: Dictionary containing the distribution layer
    ...
    def init(key):
        ...
        params  = params_init(key,distribution)
        params_all["distribution"] = params
        key,  rng = jax.random.split(key)
        params = params_init(key,control)
        params_all["control"] = params
        
        return params_all
    
    def params_init(key, setting):
