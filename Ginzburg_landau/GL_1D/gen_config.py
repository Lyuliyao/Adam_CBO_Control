...
def generate_configure(dim):
    ...
    def fcn_f(m: jnp.ndarray, x: jnp.ndarray) -> jnp.ndarray:
        return jnp.sum(m**2, axis=-1)+ jnp.sum(x**2, axis=-1)/problem_configure["dim"] 

    def fcn_g(x: jnp.ndarray) -> jnp.ndarray:
        return 10*jnp.sum(x**2, axis=-1)/problem_configure["dim"]
    # def fcn_f(m:jnp.ndarray,x: jnp.ndarray) -> jnp.ndarray:
    #     return jnp.sum(x**2, axis=-1)/problem_configure["dim"] + jnp.sum(m**2, axis=-1)
    ...
    NN_configure = {
        ...
        "output_dim": 1,
        "layers": [10*problem_configure["dim"] ,10*problem_configure["dim"] ,10*problem_configure["dim"]],
